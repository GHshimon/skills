...
## 運用メモ
- 上表は**専門（判断）エージェント**の編成。運用系の実行体（独立採点の `agents/skill-scorer.md` 等）はロスター対象外で、リポジトリ直下 `agents/` に置く。Worker スキルの実行は常設エージェントを作らず `Agent(model: sonnet)` ＋スキル発動で行う（常設は繰り返し実績が出てから）
